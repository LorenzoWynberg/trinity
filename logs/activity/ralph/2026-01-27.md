...
## Dashboard: Replace tasks.json with SQLite Database

**Time:** 2026-01-27 ~22:35 CR

### What was done

Replaced the file-based task queue (`tasks.json` with file locking) with SQLite using `better-sqlite3`. This provides proper database semantics, better concurrency, and a foundation for persistent settings.

**Why SQLite:**
- File locking was fragile (race conditions, stale locks)
- Needed proper queries (filter by status, type, date)
- Future: settings, metrics, other persistent data
- better-sqlite3 is synchronous (no async complexity)

**Database setup (`lib/db/index.ts`):**
- Singleton connection with WAL mode
- Foreign keys enabled
- Auto-migration on first connect
- Task and settings operations exported

**Migration system:**
- SQL files in `src/lib/db/migrations/`
- Applied in filename order (001_, 002_, etc.)
- Tracked in `migrations` table
- Each migration wrapped in transaction

**Initial migrations:**
- `001_tasks.sql` - Tasks table with status, type, version, params, result, error, timestamps
- Indexes on status, type, created_at

**Task operations:**
- `tasks.create(type, version, params)` - Create queued task
- `tasks.get(id)` - Get by ID with JSON parsing
- `tasks.list({ type, status, limit })` - Filtered list
- `tasks.getActive()` - Queued or running
- `tasks.getNext()` - Next queued (if nothing running)
- `tasks.start(id)` - Set to running
- `tasks.complete(id, result)` - Set to complete with result
- `tasks.fail(id, error)` - Set to failed with error
- `tasks.cleanup(keepCount)` - Delete old completed tasks

**Zustand store (`lib/store.ts`):**
- Client-side state management
- `seenTaskIds` - Track which tasks user has seen (persisted)
- `pendingTaskId` - Task awaiting modal display
- `markTaskSeen(id)` - Mark as seen
- `setPendingTask(id)` - Set for modal opening

### Files created

- `tools/ralph/dashboard/src/lib/db/index.ts` - SQLite connection + operations
- `tools/ralph/dashboard/src/lib/db/migrations/001_tasks.sql` - Initial schema
- `tools/ralph/dashboard/src/lib/store.ts` - Zustand store

### Files modified

- `tools/ralph/dashboard/package.json` - Added better-sqlite3, zustand, immer
- `tools/ralph/dashboard/src/lib/tasks.ts` - Rewrote to use db layer
- `tools/ralph/dashboard/src/components/task-provider.tsx` - Use Zustand store
- `tools/ralph/dashboard/src/components/task-indicator.tsx` - Use Zustand store
- `tools/ralph/dashboard/.gitignore` - Added dashboard.db

### Files deleted

- `tools/ralph/dashboard/tasks.json` - Replaced by SQLite

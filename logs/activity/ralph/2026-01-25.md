...
## feat: Version metadata in dropdowns and graph nodes

**Started:** 2026-01-25 ~25:00

### What was done

Added version metadata (title, shortTitle, description) display throughout the dashboard:

**Dropdowns:**
- Dashboard version selector: `v0.1 - MVP` format
- Graph page version selector: `v0.1 - MVP` format
- Stories page version selector: `v0.1 - MVP` format

**Graph version nodes:**
- Now show title below version number
- Description shown below title (2 lines max, truncated)
- Node max-width increased to 240px for readability

**Data layer:**
- Added `getVersionsWithMetadata()` function in data.ts
- Updated `VersionInfo` type to include title, shortTitle, description
- Updated `getVersionProgress()` to include metadata

### Files modified

- `tools/ralph/dashboard/src/lib/types.ts` - Added metadata fields to VersionInfo
- `tools/ralph/dashboard/src/lib/data.ts` - Added getVersionsWithMetadata, updated getVersionProgress
- `tools/ralph/dashboard/src/components/version-selector.tsx` - Accept and display metadata
- `tools/ralph/dashboard/src/components/version-node.tsx` - Show title and description
- `tools/ralph/dashboard/src/components/stories-list.tsx` - Accept and display metadata
- `tools/ralph/dashboard/src/hooks/use-graph-data.tsx` - Pass metadata to version nodes
- `tools/ralph/dashboard/src/app/page.tsx` - Fetch and pass metadata
- `tools/ralph/dashboard/src/app/stories/page.tsx` - Fetch and pass metadata
- `tools/ralph/dashboard/src/app/graph/page.tsx` - Use metadata in version dropdown
...
Made the dashboard mobile-friendly with several improvements:

### Sidebar (done earlier)
- Added collapsible hamburger menu for mobile
- Fixed hydration error with mounted state pattern

### Graph Page
- Added mobile dropdown menu for graph controls (layout, dead ends, external deps, fullscreen)
- Desktop controls hidden on mobile with `hidden md:flex`
- ReactFlow zoom/pan controls repositioned higher on mobile (bottom: 80px)

### Stories Page
- Wrapped TabsList in scrollable container for horizontal swipe on mobile
- Tabs no longer overflow the viewport

### Learnings Page
- Extracted Tabs into client component (`learnings-tabs.tsx`) to fix Radix hydration error
- Same scrollable TabsList wrapper for mobile

### ngrok Configuration
- Updated to use custom domain `ozner-ralph-dashboard.ngrok.app` (no interstitial page on paid plan)
- Removed terminal tunnel (only one domain available)

### Files modified
- `tools/ralph/dashboard/src/app/globals.css` - Mobile ReactFlow controls positioning
- `tools/ralph/dashboard/src/app/graph/page.tsx` - Mobile dropdown menu
- `tools/ralph/dashboard/src/app/learnings/page.tsx` - Use client component for tabs
- `tools/ralph/dashboard/src/components/learnings-tabs.tsx` - New client component
- `tools/ralph/dashboard/src/components/stories-list.tsx` - Scrollable tabs wrapper

---

## Documentation Updates

Added to `docs/learnings/ralph.md`:

### Hydration Issues (expanded)
- Fix 1: Suspense boundary for useSearchParams
- Fix 2: Extract Radix Tabs to client component for server components

### Mobile Responsiveness (new section)
- Scrollable tabs pattern with overflow-x-auto wrapper
- Hamburger menu mounted state pattern
- ReactFlow controls CSS positioning

### ngrok Setup (new section)
- Config file location and format
- Hobby plan limits (3 endpoints, 1 custom domain)
- Custom domain vs random URL (interstitial behavior)
- Terminal WebSocket auto-detection via /api/tunnels

---

## Terminal PTY Upgrade

Upgraded ws-server.js from simple spawn to full PTY using node-pty.

### Problem
`posix_spawnp failed` error when using node-pty with Node 22.

### Solution
Rebuild node-pty from source instead of using prebuilt binary:
```bash
rm -rf node_modules/node-pty && npm install node-pty --build-from-source
```

### Result
- Full PTY shell support (zsh)
- Interactive programs like `claude` now work
- Tab completion and command history work
- Works on both Node 20 and Node 22

### Files modified
- `tools/ralph/dashboard/ws-server.js` - Switched from spawn to node-pty

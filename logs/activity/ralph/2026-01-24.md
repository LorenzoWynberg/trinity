...
- Linear interpolation: `index = Math.round(depth * 24 / maxDepth)`
- Always uses full spectrum regardless of max depth
...
### Edge Z-Index by Pixel Length
- Z-index now based on actual visual edge length, not graph depth
- Shorter edges render on top of longer edges
- Formula: `zIndex = 10000 - edgeLength`

### Highlighted Nodes Above Edges
- Highlighted nodes have z-index 20000 (above max edge z-index of 10000)
- Ensures nodes are never obscured by edges during highlighting

---

## New Story ID Format & Versioned Node IDs

### Story ID Format Change
- Changed from `STORY-X.Y.Z` to `X.Y.Z` format
- Cleaner, more concise IDs
- Updated both v1.0 and v2.0 PRD files using jq

### Cross-Version Dependency Syntax
- Same version: `1.2.3` (direct story ID)
- Cross-version specific story: `v1.0:1.2.3`
- Cross-version whole version: `v1.0` (depends on all leaf stories)
- Phase/epic refs still work: `1` (phase), `1:2` (phase:epic)

### Versioned Node IDs in Graph
- Node IDs now include version prefix: `v1.0:1.1.1`, `v2.0:1.1.1`
- Prevents conflicts when viewing all versions (same story IDs in different versions)
- Updated `resolveDependency()` to return `{ storyId, nodeId, crossVersion }`

### Cross-Version Edge Styling (Option B)
- Specific cross-version story deps create direct story-to-story edges
- Styled orange dotted (same as version-to-version edges)
- Version-to-version edges for whole version deps (`v1.0`)

### Graph Page Updates
- Added `extractStoryId()` and `extractVersion()` helpers
- Updated `getAncestors()` and `getPathEdgesWithDepth()` for versioned node IDs
- Updated `openStoryModal()` to handle versioned node IDs

### Files Modified
- `tools/ralph/cli/prd/v1.0.json` - Updated all IDs and deps to new format
- `tools/ralph/cli/prd/v2.0.json` - Updated all IDs and deps to new format
- `tools/ralph/dashboard/src/hooks/use-graph-data.tsx` - Versioned node IDs, updated resolveDependency
- `tools/ralph/dashboard/src/app/graph/page.tsx` - Helper functions for versioned IDs

---

## Version Nodes at End of Dependency Chain

### Conceptual Change
- Version nodes now represent the END/GOAL of a version, not the start
- Stories flow INTO the version node (leaves → version)
- Version node is the "deliverable" you're working towards
- Makes more intuitive sense: complete stories → version release

### Layout Changes
- Horizontal: version nodes at far right (after all stories)
- Vertical: version nodes at bottom (below all stories)
- Version nodes always visible (not just in "all versions" view)

### Edge Direction Changes
- OLD: version → root stories (version was source)
- NEW: leaf stories → version (version is target)
- Cross-version edges: `version:v1.0 → v2.0:1.1.1` (v1.0 completion unlocks v2.0 stories)

### Leaf Story Detection Fix
- Leaf = story nothing in SAME VERSION depends on
- Cross-version deps excluded from leaf detection
- Without this fix, v1.0 stories depended on by v2.0 wouldn't connect to v1.0 node

### Highlighting Updates
- Clicking version node shows all stories flowing into it
- `getAncestors()` recursively adds all version's stories when clicking version
- `getPathEdgesWithDepth()` creates edges from leaves to version node

### Files Modified
- `tools/ralph/dashboard/src/hooks/use-graph-data.tsx`:
  - Changed `showVersionHeaders` to show when versions >= 1
  - Reversed edge direction (leaf → version instead of version → root)
  - Fixed leaf detection to exclude cross-version deps (`!r.crossVersion`)
  - Version node positioning at end of chain
- `tools/ralph/dashboard/src/app/graph/page.tsx`:
  - Updated `getAncestors()` for version-at-end model
  - Fixed `leafStoryIds` memo to exclude cross-version deps
  - Updated highlighting logic for new edge directions

---

## Per-Version Color Gradients

### Problem
- When highlighting paths spanning multiple versions (v1.0 → v2.0)
- Colors went through entire spectrum across both versions
- Made it hard to see structure within each version

### Solution
- Each version now has its own independent color gradient
- v1.0: cyan (leaves) → yellow (roots) based on v1.0's depth range
- v2.0: cyan (leaves) → yellow (roots) based on v2.0's depth range
- Cross-version edges use target version's color scheme

### Implementation
- Changed `highlightedEdges` from `Map<string, number>` to `Map<string, { depth: number; version: string }>`
- Added `maxDepthPerVersion` state to track max depth per version
- Calculate version-relative depths (subtract min depth for each version)
- `getDepthColor(depth, version)` now uses version-specific max depth

### Files Modified
- `tools/ralph/dashboard/src/app/graph/page.tsx`:
  - Updated `highlightedEdges` state type
  - Added `maxDepthPerVersion` state
  - Updated `getPathEdgesWithDepth()` to track versions and return relative depths
  - Updated `getDepthColor()` to accept version parameter
  - Updated edge styling to use version-specific colors

...
## Feature: Story handoffs UI component

**Time:** 2026-01-31 ~20:30 SAST

### What was done

Added StoryHandoffs component to story detail page showing agent pipeline visualization and handoff history.

### Features

- **Pipeline visualization**: Shows all 5 agents (analyst → implementer → reviewer → refactorer → documenter)
- **Agent status**: Blue pulse for active, green check for completed
- **Handoff history**: Expandable list with timestamps
- **Payload inspection**: Expand to view handoff payloads
- **Rejection details**: Shows rejection reasons when applicable

### Files created

- `src/components/story-handoffs.tsx`

### Files modified

- `src/app/stories/[id]/page.tsx` - Import and render StoryHandoffs

---

## Cleanup: Remove Elvish CLI

**Time:** 2026-01-31 ~20:44 SAST

### What was done

Removed the legacy Elvish CLI and related documentation. The dashboard now handles all Ralph functionality.

### Removed

- `tools/ralph/cli/` - Entire Elvish CLI directory (ralph.elv, lib/*.elv, etc.)
- `docs/gotchas/elvish/` - Elvish language gotchas
- `docs/knowledge/ralph-cli/` - CLI documentation

### Updated

- `CLAUDE.md` - Removed CLI references, updated data storage and resetProject sections
- `docs/gotchas/README.md` - Removed elvish.md from table
- `docs/knowledge/README.md` - Updated structure to reflect current directories

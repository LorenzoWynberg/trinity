...
export const VersionNode = memo(({ data }: NodeProps) => {
  ...
  return (
    <>
      ...
      >
        <div className={cn('font-bold text-lg text-purple-800 dark:text-purple-100 cyber-dark:text-cyan-100 cyber-light:text-pink-600', isVertical && 'text-center')}>
          {nodeData.label}
        </div>
        {nodeData.title && (
          <div className={cn('mt-1 text-sm font-medium text-purple-700 dark:text-purple-200 cyber-dark:text-fuchsia-200 cyber-light:text-cyan-600', isVertical && 'text-center')}>
            {nodeData.title}
          </div>
        )}
        {nodeData.description && (
          <div className={cn('mt-1 text-xs text-purple-500 dark:text-purple-400 cyber-dark:text-fuchsia-300 cyber-light:text-pink-400', isVertical && 'text-center')}>
            {nodeData.description}
          </div>
        )}
